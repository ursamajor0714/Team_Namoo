...
/**
 * 겪은 문제. 사소한 실수 대신 설계 판단이 걸린 것만 남긴다.
 * 전문 용어를 쓰되 바로 다음 문장에서 일상어로 풀어 준다.
 */
const TROUBLES = [
  {
    title: '채점 기준과 학습 기준이 20배 어긋나 있었다',
    detail:
      '모델을 채점할 때는 드문 성향을 맞히는 데 높은 배점을 줬는데, 정작 학습할 때 모델에게 알려주는 중요도는 전혀 다른 비율이었습니다. 시험은 서술형으로 보면서 공부는 객관식으로 하고 있던 셈입니다. 학습 중요도를 손으로 정하지 않고 채점 기준에서 계산해 뽑도록 바꾼 것이 이번 작업에서 가장 큰 개선이었습니다.',
  },
  {
    title: '정확도라는 지표 자체가 함정이었다',
    detail:
      '전체 기사의 48.5%가 "정치와 무관"입니다. 그래서 무엇을 넣든 무조건 "판단불가"라고만 답해도 정확도가 절반 가까이 나옵니다. 실력이 아니라 요령인데 숫자로는 구분이 안 됩니다. 그래서 드문 성향을 맞히면 크게 쳐주고 놓치면 크게 깎는 점수판을 따로 만들었습니다. 배점은 사람이 정하지 않고 각 성향이 얼마나 드문지에서 자동으로 계산되게 했습니다.',
  },
  {
    title: '틀린 문제만 다시 풀렸더니 이미 알던 것까지 잊었다',
    detail:
      '오답만 모아 다시 학습시키면 좋아질 줄 알았는데 정확도가 54%에서 20%로 무너졌습니다. 새로 배운 것이 이전 지식을 덮어쓰는 현상입니다. 맞힌 문제를 반드시 함께 섞어야 했습니다.',
  },
  {
    title: '데이터를 조각내 학습한 실험 설계가 애초에 틀렸다',
    detail:
      '기사를 여러 묶음으로 나눠 순서대로 학습시키며 성능을 비교하고 있었습니다. 그런데 뒤 묶음이 앞 묶음을 덮어써서, 사실상 마지막 묶음만 배운 모델을 매번 채점하고 있던 것입니다. 며칠치 실험 결과가 통째로 의미가 없었습니다. 전체를 한 번에 학습시키자 54%에서 68%로 올랐습니다.',
  },
  {
    title: '읽는 길이를 늘리자 메모리가 제곱으로 늘었다',
    detail:
      '기사를 더 길게 읽히면 정확도가 오를 것 같아 읽는 분량을 2배로 늘렸습니다. 그런데 필요한 메모리는 2배가 아니라 4배가 됩니다. 모델이 단어끼리 서로를 비교하는 방식이라 길이가 늘면 비교 횟수가 제곱으로 늘어나기 때문입니다. 모르고 돌렸다가 39분을 날렸습니다.',
  },
]
