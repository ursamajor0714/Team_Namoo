...
public class ArticleTextExtractor {
    ...
    private static final String NOISE_SELECTORS = String.join(",",
            ...
            // 읽기모드/다크모드/폰트크기/북마크/프린트 같은 기사 상단 유틸리티 툴바(언론사마다 클래스명이 제각각이라
            // 자주 쓰이는 토큰들을 모아둔다)
            "[class~=(?i)\\b(util|function|func|tool|toolbar|option|control|fontsize|font_size|textsize|text_size|darkmode|dark_mode|readmode|read_mode|bookmark)\\b]",
            "[id~=(?i)\\b(util|function|func|tool|toolbar|option|control|fontsize|font_size|textsize|text_size|darkmode|dark_mode|readmode|read_mode|bookmark)\\b]"
    );

    /**
     * 위 CSS 셀렉터로 못 거른, 클래스/아이디가 없거나 사이트마다 제각각인 툴바 라벨을 텍스트 기준으로 한 번 더 거른다.
     * ("가 가 가 가 가 가"처럼 폰트크기 버튼이 글자만 남는 경우도 있어 별도 처리한다.)
     */
    private static final java.util.Set<String> UI_BOILERPLATE_LABELS = java.util.Set.of(
            "읽기모드", "다크모드", "폰트크기", "글자크기", "북마크", "프린트", "기사반응",
            "읽기 모드", "다크 모드", "폰트 크기", "글자 크기"
    );
    ...
    public ExtractedArticle extract(String url) throws IOException {
        ...
        content = stripUiBoilerplate(content);
    ...
    private ReadabilityResult extractContentWithReadability(String url, String rawHtml) {
        try {
            ...
            String safeHtml = "";
            if (rawContentHtml != null) {
                Document fragment = Jsoup.parseBodyFragment(rawContentHtml, url);
                removeUiBoilerplate(fragment.body());
                safeHtml = Jsoup.clean(fragment.body().html(), url, Safelist.relaxed().addAttributes("img", "src", "alt"));
            }
    ...
    /**
     * 텍스트에서 읽기모드/다크모드/폰트크기/북마크/프린트/기사반응 같은 툴바 라벨 줄을 제거한다.
     * NOISE_SELECTORS가 클래스/아이디로 못 거른 경우를 위한 마지막 안전망.
     */
    private String stripUiBoilerplate(String text) {
        ...
        for (String line : text.split("\n")) {
            ...
            if (trimmed.isEmpty() || isUiBoilerplateLine(trimmed)) {
                continue;
            }
            if (result.length() > 0) {
                result.append("\n");
    ...
    private boolean isUiBoilerplateLine(String trimmed) {
        if (UI_BOILERPLATE_LABELS.contains(trimmed)) {
            return true;
        }
        // 폰트크기 버튼이 "가 가 가 가 가 가"처럼 글자만 남아 텍스트로 뽑히는 경우
        return trimmed.chars().allMatch(c -> c == '가') && trimmed.length() <= 10;
    }

    /**
     * HTML 조각에서 위와 같은 툴바 라벨을 가진 작은 요소(li/span/button/a 등)를 제거한다.
     * 모든 자식이 제거된 목록(ul/ol)은 빈 목록으로 남지 않도록 통째로 제거한다.
     */
    private void removeUiBoilerplate(Element root) {
        Elements candidates = root.select("li, span, button, a, p, div");
        for (Element el : candidates) {
            if (isUiBoilerplateLine(el.ownText().trim())) {
                el.remove();
            }
        }
        Elements lists = root.select("ul, ol");
        for (Element list : lists) {
            if (list.children().isEmpty() && list.text().isBlank()) {
                list.remove();
            }
        }
    }
